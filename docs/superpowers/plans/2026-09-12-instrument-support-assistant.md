...
- Generation, judge, and corpus-generation LLM: Gemini Flash-Lite (rolling alias), model ID `gemini-flash-lite-latest`, called via the Gemini API's free tier — the whole project runs at $0 API cost. One model used everywhere (no cost-driven tiering) since the free tier is what makes cost irrelevant. (Originally `gemini-3.8-flash`; switched after discovering its free tier has a ~20-request daily quota, too tight for this project's real corpus-generation and eval-ablation call volumes. A first attempt to pin `gemini-2.5-flash-lite` failed outright — Google had already retired it for new users by the time this project ran — so the rolling `-latest` alias is used instead of a dated version, to avoid re-hitting this same class of failure as models are retired over time. See the "Model downgrade" note in Self-Review Notes.)
...
`_write_if_missing` makes the script resumable: the real Gemini API free tier for `gemini-3.8-flash` has a short-window rate limit (observed: 20 requests before a 429 with a ~48s retry hint), so a real run may need multiple invocations. Skipping already-written files means a retry after a rate-limit backoff doesn't re-spend quota on models that already succeeded.
...
- **Model downgrade (during execution, while running the real corpus generation for the first time):** `gemini-3.8-flash`'s free tier turned out to have a ~20-request daily quota (confirmed empirically: after the quota was hit, waiting nearly an hour with retries produced zero additional successes, and a subsequent attempt a day later also failed, this time with a transient 500 "high demand" error) — far too tight for this project's real corpus-generation (~16 calls) and eval-ablation (~240 calls) volumes. First attempted to switch all three model constants in `core/config.py` (Task 1) to a pinned `gemini-2.5-flash-lite`, but the real API rejected it outright with a 404: "This model models/gemini-2.5-flash-lite is no longer available to new users" — Google had already retired it. Switched again to the rolling alias `gemini-flash-lite-latest` instead of another pinned version, specifically to avoid re-hitting this same failure mode as dated models get retired over time, and updated the Global Constraints description to match. Test files in Tasks 11-14 that pass `"gemini-3.8-flash"` as an arbitrary literal string parameter (not read from `core.config`) were left unchanged — those tests validate model-string pass-through generically and don't depend on which literal is used, so this wasn't worth another review cycle across four already-approved tasks purely for cosmetic consistency. Task 1's own verification that `gemini-3.8-flash` is a real, listed model in the SDK's `Model` type remains historically accurate and was left as-is.
